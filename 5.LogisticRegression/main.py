...
def step1_get_data():
    ...
    names = iris.target_names[
        :100
    ]  # found input variables with inconsistent numbers of samples: [100, 3] 나왔음
    # print(X)
    # print(y)
    # print(names)
    return X, y
...
def step2_learning():
    ...
    ml = LogisticRegression(
        random_state=0, C=1000.0, max_iter=10
    )  # c 값은 데이터 양에 따라서 값이 변경
...
def step3_using():
    with open("5.LogisticRegression/model.pkl", "rb") as fp:
        sc = pickle.load(fp)
        ml = pickle.load(fp)
    X = [[1.4, 0.2], [1.5, 0.3], [1.6, 0.4], [1.7, 0.5], [1.8, 0.6]]  # 콤마 안 붙여서  오류
